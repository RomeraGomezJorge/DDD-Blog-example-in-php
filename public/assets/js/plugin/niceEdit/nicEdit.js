...
var nicEditorConfig = bkClass.extend({
    ...
    iconsPath: 'https://i.imgur.com/FZsqKv3.gif',
...
var nicEditor = bkClass.extend({
    ...
    panelInstance: function(B, C) {
        ...
        var A = new bkElement("DIV").setStyle({ width: "100%" }).appendBefore(B);
        this.setPanel(A);
...
var nicEditorInstance = bkClass.extend({
    ...
    construct: function(G, D, C) {
        ...
        if (H || this.options.hasPanel) {
            ...
            var E = { width: "100%", border: "1px solid #ccc", borderTop: 0, overflowY: "auto", overflowX: "hidden" };
            ...
            var A = new bkElement("DIV").setStyle({ width: "98%", margin: "4px", minHeight: newY + "px" }).addClass("main").appendTo(this.editorContain);
...
var nicEditorIFrameInstance = nicEditorInstance.extend({
    ...
    init: function() {
        ...
        if (this.copyElm) { this.elmFrame.setStyle({ width: "99%" }) }
...
var nicUploadButton = nicEditorAdvancedButton.extend({
    ...
    onUploaded: function(B) {
        this.removePane(); /*var D=B.links.original;*/
        var D = B['url'];
        if (!this.im) {
            this.ne.selectedInstance.restoreRng();
        ...
        if (this.im) { this.im.setAttributes({ src: D, width: (A && B.width) ? Math.min(A, B.image.width) : "" }) }
    }
});
...
nicEditor = nicEditor.extend({
    ...
    reposition: function() {
        ...
        this.floating.setStyle({ width: (parseInt(B.getStyle("width")) || B.clientWidth) + "px" });
        var A = B.offsetTop - this.floating.offsetHeight;
        if (A < 0) { A = B.offsetTop + B.offsetHeight }
        this.floating.setStyle({ top: A + "px", left: B.offsetLeft + "px", display: "block" })
    },
    hide: function() { this.floating.setStyle({ top: "-1000px" }) }
});
...
// Plugin para insertar codigo html donde se encuentre el cursor
...
var nicMyhtmlOptions = {
    ...
    iconFiles: { 'html': 'https://i.imgur.com/5ppeQkA.gif' }

};

nicMyhtmlButton = nicEditorAdvancedButton.extend({
    addPane: function() {
        this.addForm({
